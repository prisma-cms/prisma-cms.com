...
describe('@prisma-cms/boilerplate users page test', () => {
  ...
  it('Render users page', () => {
    render(
      <Renderer lang="ru" pathname={pathname} title="Users"></Renderer>,
      node,
      () => {
        const {
          document: { title, status },
  ...
  const userPageUrl = '/users/cjmtayev000ex0a5028xzq7b1'

  if (userPageUrl) {
    it('Render user page', () => {
      render(
        <Renderer lang="ru" pathname={userPageUrl}></Renderer>,
        node,
        () => {
          const {
            document: { title, status },
