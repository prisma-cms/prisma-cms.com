...
export class UserTechnologyProcessor extends PrismaProcessor {
  ...
  async mutate(method, args, info) {
    ...
    if (args.data) {
      let { Technology, date_from, date_till, ...data } = args.data

      const { where } = args

      const userTechnology_old = where
        ? await db.query.userTechnology({
            where,
          })
        : null
      ...
      if (date_from !== undefined || date_till !== undefined) {
        ...
        if (date_from && moment(new Date(date_from)) > moment()) {
          this.addFieldError(
            'date_from',
            'Дата С не может быть больше текущей даты'
          )
        }
        ...
        if (date_till && moment(new Date(date_till)) > moment()) {
          this.addFieldError(
            'date_till',
            'Дата До не может быть больше текущей даты'
          )
        }
        ...
        if (
          date_from_value &&
          date_till_value &&
          moment(new Date(date_from_value)) > moment(new Date(date_till_value))
        ) {
