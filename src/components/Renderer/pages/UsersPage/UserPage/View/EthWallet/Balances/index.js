...
export const locales = {
  ru: {
    values: {
      ...
      Attention: 'Внимание',
      'We are not liable for any transactions and losses from ethereum wallets.':
        'Мы не несем никакой ответственности за какие-либо операции и потери с ethereum-кошельков.',
      ...
      'The address will be determined by the private key':
        'По приватному ключу будет получен адрес',
      ...
      'The received address does not match the address of your wallet':
...
export class WalletBalances extends PrismaCmsComponent {
  ...
  async sendEth() {
    const {
      query: { createEthTransactionProcessor },
    } = this.context
  ...
  render() {
    ...
    const { user: currentUser, TransactionLink, Grid } = this.context
          ...
          if (showTopupBalanceForm) {
            actions.push(
              <div key="topupBalance">
                ...
                <Typography className={classes.paragraph}>
                  {this.lexicon('We advise to use the service')}{' '}
                  <a
          ...
          {showInfo ? (
            <div>
              <Typography className={classes.paragraph}>
                ...
                ({this.lexicon('Read more')}{' '}
                <a
                  href="https://ru.wikipedia.org/wiki/Ethereum"
                  target="_blank"
                  rel="nofollow"
                >
                  {this.lexicon('in wikipedia')}
                </a>
                ).
              </Typography>
