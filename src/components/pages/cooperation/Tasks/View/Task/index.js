...
export class TaskView extends EditableView {
  ...
  async saveObject(data) {
    let { mutate, createTask, updateTask } = this.props
  ...
  getButtons() {
    ...
    const { createTimer, updateTimer, classes } = this.props
  ...
  renderHeader() {
    ...
    const { id: taskId, CreatedBy, createdAt, Project } = object || {}
  ...
  renderActiveTimers() {
    ...
    if (activeTimers.length) {
      activeTimers.map((n) => {
        ...
        collaborators.push(
          <Grid key={id} item>
            <UserLink user={CreatedBy} size="small" />
          </Grid>
        )
      })
    }
  ...
  renderDefaultView() {
    const { classes, showDetails } = this.props
