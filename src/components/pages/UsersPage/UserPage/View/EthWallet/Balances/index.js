...
export class WalletBalances extends PrismaCmsComponent {
  ...
  async importWallet() {
    const { createInRequest, ethWalletPK, ethWalletPKSendEmail } = this.state
  ...
  async createWallet() {
    ...
    if (!account) {
      ...
      const { privateKey: ethWalletPK } = account
  ...
  checkWallet() {
    ...
    const { ethWalletPKCheck } = this.state
    ...
    if (ethWalletPKCheck && !/^0x/.test(ethWalletPKCheck)) {
      ...
      if (account) {
        if (address !== account.address) {
          errorMessage =
            'Полученный адрес не совпадает с адресом вашего кошелька'
          checkStatus = false
        } else {
          checkStatus = true
        }
  ...
  async sendEth() {
    const { amount, sendEthInRequest, sendPrivateKey: privateKey } = this.state
    ...
    await this.mutate({
      ...
      .then((result) => {
        ...
        if (response) {
          const { success, data: transaction } = response
  ...
  render() {
    const { user, ethAccount, classes, children, ...other } = this.props

    const { user: currentUser, TransactionLink } = this.context

    const { address, balance } = ethAccount || {}
    ...
    const { id: currentUserId, EthAccounts: currentUserEthAccounts } =
      currentUser || {}

    const currentUserEthAccount =
      (currentUserEthAccounts && currentUserEthAccounts[0]) || null

    const { balance: currentUserBalance } = currentUserEthAccount || {}
    ...
    if (ethAccount) {
      output = (
        <Fragment>
          ...
          <Typography>Баланс Eth: {balance}</Typography>
        </Fragment>
      )
      ...
      if (currentUserId) {
        if (currentUserId === userId) {
          ...
          if (showTopupBalanceForm) {
            actions.push(
              <div key="topupBalance">
                <Typography variant="subheading" color="secondary">
                  Внимание!
                </Typography>

                <Typography className={classes.paragraph}>
                  Пополняется ваш личный этериум-кошелек. Баланс будет
                  отображаться на сайте Клуба, но мы никак не контролируем ваш
                  кошелек и действия с ним.
                </Typography>

                <Typography className={classes.paragraph}>
                  Введите ваш приватный ключ, чтобы убедиться, что у вас есть
                  доступ к вашему кошельку. Иначе вы переведете средства и не
                  сможете их потом забрать.
                </Typography>
                ...
                <Typography className={classes.paragraph}>
                  Мы не принимаем оплату на пополнение кошелька. Для пополнения
                  вы можете выбрать любой сервис для покупки криптовалюты
                  ethereum.
                </Typography>

                <Typography className={classes.paragraph}>
                  Мы советуем воспользоваться сервисом{' '}
                  <a
                  ...
                  , так как у них минимальная сумма пополнения всего 0.01
                  этериума, и сами оплачивали, они не обманывали.
                </Typography>
              </div>
            )
          }
          ...
          if (sendSuccessTransaction) {
            const { id } = sendSuccessTransaction

            actions.push(
              <div key="ethSendedSuccess">
                <Typography className={classes.textSuccess}>
                  Перевод успешно выполнен.
                </Typography>

                <Typography>
                  Посмотреть данные транзакции можно{' '}
                  <TransactionLink object={sendSuccessTransaction}>
                    здесь
                  </TransactionLink>
                  .
                </Typography>
          ...
          case 'createForm':
            form = (
              <div className={classes.paragraph}>
                <Typography>
                  Для вас будет создан новый ethereum-кошелек. В базе сохранится
                  только его публичный адрес, без каких-либо ключей доступа.
                </Typography>
      ...
      output = (
        <div>
          <Typography className={classes.paragraph}>
            У вас еще нет кошелька, но вы можете его{' '}
            <a
              href="javascript:;"
              onClick={(event) => {
          ...
          {form ? (
            <Typography className={classes.paragraph}>
              ...
              MODX-Клуб не несет никакой ответственности за какие-либо операции
              и потери с ethereum-кошельков.
            </Typography>
          ) : null}
          ...
          {showInfo ? (
            <div>
              <Typography className={classes.paragraph}>
                Данный кошелек не является кошельком в прямом смысле слова, а
                является ссылкой на ваш личный адрес в системе{' '}
                <a
              ...
              <Typography className={classes.paragraph}>
                Если вы не знаете точно, что это такое и как работает, вам лучше
                не стоит его заводить сейчас, а попытаться получше узнать о этой
                системе.
              </Typography>

              <Typography className={classes.paragraph}></Typography>

              <Typography color="secondary" className={classes.paragraph}>
                Мы не храним ваши приватные ключи. Ключ нужен только для того,
                чтобы получить адрес вашего кошелька и убедиться, что у вас есть
                к нему доступ. Никому ключ не будет показан.
              </Typography>
            </div>
          ) : null}
        </div>
